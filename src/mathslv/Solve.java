...
//                  gets the powers in reversed order (because they are evaluated that way)
                    ...
                    while (powers.size() != 1) {
                        double p1 = Double.parseDouble(powers.pop().replace(",", "."));
                        double p = Double.parseDouble(powers.pop().replace(",", "."));
...
//          equates the multiplication and division
            while (higher[i].size() != 1) {
                ...
                if (action == '*') {
                    higher[i].add(0, String.valueOf(Double.parseDouble(higher[i].remove(0).replace(",", ".")) * Double.parseDouble(higher[i].remove(0).replace(",", "."))));
                } else if (action == '/') {
                    higher[i].add(0, String.valueOf(Double.parseDouble(higher[i].remove(0).replace(",", ".")) / Double.parseDouble(higher[i].remove(0).replace(",", "."))));
                }
            }
...
//      equates the addition and subtraction
        while (lower.size() > 1) {
            ...
            if (action == '+') {
                lower.add(0, String.valueOf(Double.parseDouble(lower.remove(0).replace(",", ".")) + Double.parseDouble(lower.remove(0).replace(",", "."))));
            } else if (action == '-') {
                lower.add(0, String.valueOf(Double.parseDouble(lower.remove(0).replace(",", ".")) - Double.parseDouble(lower.remove(0).replace(",", "."))));
            }
        }
//      parses the result to double and sets precision 6 digits
        ...
        String s = String.format("%.6f", result).replace(",", ".");
        return s;
    }
